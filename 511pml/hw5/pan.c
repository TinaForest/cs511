...
short src_ln1 [] = {
	  0,  39,  40,  41,  43,  44,  44,  45, 
	 46,  47,  39,  49,  39,  37,  51,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "crlea.pml", 1, 14 },
	{ "-", 15, 16 }
...
short src_ln0 [] = {
	  0,   9,  11,  12,  14,  15,  16,  17, 
	 18,  19,  20,  13,  22,  22,  23,  24, 
	 26,  27,  28,  29,  30,  25,  32,  10, 
	 34,  10,  34,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "crlea.pml", 1, 26 },
...
{	int j = 0, h = now._nr_pr;
	...
	case 1:	/* :init: */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval(":init::i", ((P1 *)pptr(h))->_4_1_i);
		logval(":init::id", ((P1 *)pptr(h))->_4_1_id);
		logval(":init::next_id", ((P1 *)pptr(h))->_4_1_next_id);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* nnode */
		...
		/* locals: */
		((P0 *)pptr(h))->leader_id = 0;
		((P0 *)pptr(h))->eId = 0;
		((P0 *)pptr(h))->lId = 0;
#ifdef VAR_RANGES
		logval("nnode:id", ((P0 *)pptr(h))->id);
		logval("nnode:next_id", ((P0 *)pptr(h))->next_id);
		logval("nnode:leader_id", ((P0 *)pptr(h))->leader_id);
		logval("nnode:eId", ((P0 *)pptr(h))->eId);
		logval("nnode:lId", ((P0 *)pptr(h))->lId);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
		...
		now.ch[3] = addqueue(calling_pid, 4, 0);
		now.ch[4] = addqueue(calling_pid, 5, 0);
#ifdef VAR_RANGES
#endif
}
...
{	/* int i; */
	...
	c_chandump(now.ch[2]);
	printf("	chan ch[3] (=%d):	len %d:\t", now.ch[3], q_len(now.ch[3]));
	c_chandump(now.ch[3]);
	printf("	chan ch[4] (=%d):	len %d:\t", now.ch[4], q_len(now.ch[4]));
	c_chandump(now.ch[4]);
}
void
...
{	/* int i; */
	...
	printf("	byte   i:	%d\n", ((P1 *)pptr(pid))->_4_1_i);
	printf("	byte   id:	%d\n", ((P1 *)pptr(pid))->_4_1_id);
	printf("	byte   next_id:	%d\n", ((P1 *)pptr(pid))->_4_1_next_id);
		break;
	case 0:
		printf("local vars proc %d (nnode):\n", pid);
	printf("	byte   id:	%d\n", ((P0 *)pptr(pid))->id);
	printf("	byte   next_id:	%d\n", ((P0 *)pptr(pid))->next_id);
	printf("	byte   leader_id:	%d\n", ((P0 *)pptr(pid))->leader_id);
	printf("	byte   eId:	%d\n", ((P0 *)pptr(pid))->eId);
	printf("	byte   lId:	%d\n", ((P0 *)pptr(pid))->lId);
		break;
	}
