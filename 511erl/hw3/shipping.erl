...
get_container(Shipping_State, Container_ID) ->
    Container=lists:keyfind(Container_ID,#container.id,Shipping_State#shipping_state.containers),
    case Container of
        false -> throw(error);
        _ -> Container
    end.
...
get_port(Shipping_State, Port_ID) ->
    Port=lists:keyfind(Port_ID,#container.id,Shipping_State#shipping_state.ports),
    case Port of
        false -> throw(error);
        _ -> Port
    end.
...
get_occupied_docks(Shipping_State, Port_ID) ->
    ...
    [Dock||{PId,Dock,_ShipId}<-L,PId==Port_ID].

get_ship_location(Shipping_State, Ship_ID) ->
    L=Shipping_State#shipping_state.ship_locations,
    R=lists:keyfind(Ship_ID,3,L),
    case R of
        false-> throw(error);
        _R-> 
            {P,D,_S}=R,
...
get_container_weight(Shipping_State, Container_IDs) ->
    W=Shipping_State#shipping_state.containers,
    Weight=[X#container.weight||X<-W,Id<-Container_IDs,X#container.id==Id],
    L1=lists:flatlength(Weight),
    L2=lists:flatlength(Container_IDs),
    if L1==L2 -> lists:sum(Weight);
        true -> throw(error)
    end.
...
get_ship_weight(Shipping_State, Ship_ID) ->
    get_ship(Shipping_State, Ship_ID),
    SI=Shipping_State#shipping_state.ship_inventory,
    {ok,L}=maps:find(Ship_ID,SI),
    get_container_weight(Shipping_State,L).
    

load_ship(Shipping_State, Ship_ID, Container_IDs) ->
    % In the case that Ship ID does not exist
    % over capacity, throw an error
    % there are container IDs that aren’t in the same port as the ship 
    % if the containers are already on the ship
    Ship=get_ship(Shipping_State,Ship_ID),
    % P - port of ship
    {P,_D}=get_ship_location(Shipping_State,Ship_ID),
    SI=Shipping_State#shipping_state.ship_inventory,
    PI=Shipping_State#shipping_state.port_inventory,
    {ok,I_pi}=maps:find(P,PI),
    {ok,I_si}=maps:find(Ship_ID,SI),
    N=lists:flatlength(Container_IDs++I_si),
    io:format("~w~n",[N]),
    C1=lists:any(fun (Elem) -> lists:member(Elem, I_si) end, Container_IDs),
    C2=not(is_sublist(I_pi,Container_IDs)),
    C3=(N>Ship#ship.container_cap),
    if C1 or C2 or C3 ->
        throw(error);
    true ->
        M1=maps:put(P,I_pi--Container_IDs,PI),
        M2=maps:put(Ship_ID,I_si++Container_IDs,SI),
        {ok,Shipping_State#shipping_state{port_inventory=M1,ship_inventory=M2}}
    end.
...
unload_ship_all(Shipping_State, Ship_ID) ->
    ...
    {ok,I_pi}=maps:find(P,PI),
    {ok,I_si}=maps:find(Ship_ID,SI),
    N=lists:flatlength(I_si++I_pi),
    C=(N>Port#port.container_cap),
    if C ->
        throw(error);
    true ->
        M1=maps:put(P,I_pi++I_si,PI), 
        M2=maps:put(Ship_ID,[],SI),
        {ok,Shipping_State#shipping_state{port_inventory=M1,ship_inventory=M2}}
    end.
...
unload_ship(Shipping_State, Ship_ID, Container_IDs) ->
    ...
    {ok,I_pi}=maps:find(P,PI),
    {ok,I_si}=maps:find(Ship_ID,SI),
    N=lists:flatlength(Container_IDs++I_pi),
    C1=lists:any(fun (Elem) -> lists:member(Elem, I_pi) end, Container_IDs),
    C2=not(is_sublist(I_si,Container_IDs)),
    C3=(N>Port#port.container_cap),
    if C1 or C2 or C3 ->
        throw(error);
    true ->
        M1=maps:put(P,I_pi++Container_IDs,PI),
...
set_sail(Shipping_State, Ship_ID, {Port_ID, Dock}) ->
    ...
    {P,_D}=get_ship_location(Shipping_State,Ship_ID),
    C1=(P==Port_ID),
    SL=Shipping_State#shipping_state.ship_locations,
    % check whether (Port_ID, Dock) is occupied
    C2=lists:any(fun ({P_occupied,D_occupied,_S}) -> {P_occupied,D_occupied}=={Port_ID,Dock} end, SL),
    if C1 or C2 ->
        throw(error);
    true->
        SL_tmp=[{P_o,D_o,S_o}||{P_o,D_o,S_o}<-SL,S_o/=Ship_ID],
        SL_new=lists:append(SL_tmp,[{Port_ID,Dock,Ship_ID}]),
        {ok,Shipping_State#shipping_state{ship_locations=SL_new}}
    end.
